<?php
/**
 * Model class BPrices for SilverJet BareBone.
 *
...
class ModelVisBPrices extends Model {
...
select ID from ".$prefix."inkoop_vluchten, inkoop_vluchten_prijzen where 
VERTREK like '".strtoupper($_REQUEST[vertrek])."' AND
BESTEMMING like '".strtoupper($_REQUEST[bestemming])."' AND 
CARRIER like '".strtoupper($_REQUEST[carrier])."' AND 
KLASSE like '".strtoupper($_REQUEST[klasse])."%' AND
f_id=id and departure = '".$_REQUEST[van_jaar]."-".$_REQUEST[van_maand]."-".$_REQUEST[van_dag]."' order by price ASC
	...
	*/
		...
		if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			...
			$sql .= " ORDER BY CAST(price AS DECIMAL)";	
		}
