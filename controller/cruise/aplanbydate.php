...
class ControllerCruiseAPlanByDate extends Controller {
	private $error = array(); 
     
  	public function index() {
		$this->load->language('cruise/aplanbydate');
    	
		$this->document->setTitle($this->language->get('heading_title')); 
		
		$this->load->model('cruise/aplanbydate');
  	...
  	private function getForm() {
		...
		$this->data['tab_planbydate'] = $this->language->get('tab_planbydate');

		$this->data['entry_departure'] = $this->language->get('entry_departure');
		$this->data['entry_arrival'] = $this->language->get('entry_arrival');
    	...
    	$this->data['button_search'] = $this->language->get('button_search');
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('cruise/aplanbydate', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		...
		$this->data['departure'] = date('d-m-Y', time());
		$this->data['arrival'] = date('d-m-Y', strtotime('+ 18 months', time()));
												
		$this->template = 'cruise/aplanbydate_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function getrouteinfo(){
		$date_from = $this->request->get['dep_date'];
		$date_untill = $this->request->get['arr_date'];
		...
		$output = '';
		$resus = $this->model_cruise_aplanbydate->getCruisesByDate($date_from, $date_untill);
		...
		if($resus){
			...
			foreach($resus as $resu){
				$route_arr = array();
				$route_arr = $this->model_cruise_route->getRoute($resu['route_id']);
				
				$ship_arr = array();
				$ship_arr = $this->model_cruise_ship->getShip($route_arr['ship_id']);
				$ship_descr = $this->model_cruise_ship->getShipDescription($route_arr['ship_id']);
				...
				$output .= '</div>';
				$output .= '</div>';
				$output .= ' '. $ship_arr['ship_name'].'</td>
				';
				$output .= '<td>'. $route_arr['route_title'].'</td>';
				$output .= '<td>'. $route_arr['port_departure'].'</td>';
				$d1 = explode('-', $resu['date_departure']);
				$dep_date = date('d-m-Y', gmmktime(0,0,0,$d1[1],$d1[2],$d1[0]));
				$output .= '<td>'.$dep_date.'</td>';
				$output .= '<td>'. $route_arr['port_arrival'].'</td>';
				$d2 = explode('-', $resu['date_arrival']);
				$arr_date = date('d-m-Y', gmmktime(0,0,0,$d2[1],$d2[2],$d2[0]));
				$output .= '<td>'.$arr_date.'</td>';
				if($this->checkCabins($resu['cruise_id'])){
					$output .= '<td><a onclick="getCabins('. $resu['cruise_id'] .','.$i.')">'.$this->language->get('text_view_cabins').' <img src="view/image/magnifier.png" /></a></td>';
				}else{
	...
	public function getcabinsinfo(){
		...
		$output = '';
		$i=10000;
		$resus = $this->model_cruise_aplanbydate->getCabinsByCruiseId($cruise_id);
		if($resus){
			...
			foreach($resus as $resu){
				...
				$output .= '<img class="infoimg" src="view/image/information.png" height=16 rel="#info_'.$i.'" />';
				$output .= '<div class="simple_overlay" id="info_'.$i.'">';
				$output .= '<div class="details">'. html_entity_decode($this->model_cruise_aplanbydate->getCabinDescrByCabinId($resu['cabin_id'])) .'</div>';
				$output .= '</div>';
				$output .= ' '.$this->model_cruise_aplanbydate->getCabinTypeByCabinId($resu['cabin_id']).'</td>';
				$output .= '<td>'. $this->model_cruise_aplanbydate->getCabinCategoryByCabinId($resu['cabin_id']).'</td>';
				if($resu['price']<1){
	...
	private function checkCabins($cruise_id){
  	...
  	private function validateForm() { 
    	if (!$this->user->hasPermission('modify', 'cruise/aplanbydate')) {
